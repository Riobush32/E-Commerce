<div x-data="{ avatar: false }" @click.outside="if(avatar) avatar = false" class="bg-slate-100">
    ...
    @else
        <div class="mt-2 absolute z-[999] right-24 max-h-[50vh] overflow-auto rounded-xl" x-show="avatar"
            ...
            <div
                class="px-10 py-5 w-full max-w-sm bg-slate-700 border border-gray-200 rounded-lg shadow">

                <div class="flex flex-col items-center ">
                    <img class="w-24 h-24 mb-3 rounded-full shadow-lg"
                        src="https://tse4.mm.bing.net/th?id=OIP.ZE2YEvUMEQOyT13WVMHKtwAAAA&pid=Api&P=0&h=180"
                        alt="Bonnie image" />
                    <h5 class="mb-1  text-xl font-medium text-white ">{{ Auth::user()->name }}</h5>
                    <a href="{{ route('myVoucher') }}" class="mb-1 text-xl font-medium text-white ">{{ number_format(Auth::user()->poin)  }}<i class="fa-solid fa-coins text-yellow-400"></i></a>
